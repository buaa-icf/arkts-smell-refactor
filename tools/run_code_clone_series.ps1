...
$dataset = 'C:\Users\RoyCai\Desktop\group_project\arkts-code-smell\dataset\positive\instrument-test\code-clone.json'
  ...
  foreach ($relativePath in $changes) {
    ...
    if (-not (Test-Path -LiteralPath $baseline)) {
      ...
      if (Test-Path -LiteralPath $source) {
        throw "Removal verification failed for $relativePath."
      }
    ...
    if ($restoredHash -ne $baselineHash) {
      throw "Restore verification failed for $relativePath."
    }
    Write-Output "RESTORED=$relativePath"
  }
}

if ($StartOrdinal -lt 1 -or $EndOrdinal -lt $StartOrdinal) {
  throw 'Ordinal range is invalid.'
}
...
if ($EndOrdinal -gt $flattened.Count) {
  throw "Dataset contains only $($flattened.Count) flattened smells."
}

Set-Location -LiteralPath $homecheckParent
Write-Output "FLATTENED count=$($flattened.Count) sixthPath=$(($flattened[5]).filePath)"
...
for ($ordinal = $StartOrdinal; $ordinal -le $EndOrdinal; $ordinal++) {
  ...
  Write-Output "INPUT ordinal=$ordinal bytes=$([System.Text.Encoding]::UTF8.GetByteCount($selection))"
  $inputFile = Join-Path $env:TEMP "arkts-code-clone-input-$PID-$ordinal.json"
  ...
  if (-not $run) {
    throw "No run directory found for ordinal $ordinal."
  }
  ...
  if (-not (Test-Path -LiteralPath $resultFile)) {
    throw "Task $ordinal ended without result.json."
  }
  ...
  if (-not $sourceRoot) {
    throw "No Harmony project root found for ordinal $ordinal."
  }
