...
def _auto_config(
    ...
    if (smoke_plan or {}).get("enabled") and tools["hvigorw"] and harmony_root:
        runtime_command = [
            sys.executable, "-m", "arkts_smell_refactor.gate", "runtime-smoke",
            "--task-dir", "{task_dir}", "--source-root", str(harmony_root),
            "--hvigorw", str(tools["hvigorw"]),
        ]
        if tools.get("ohpm"):
            runtime_command.extend(["--ohpm", str(tools["ohpm"])])
        runtime = {
            "command": runtime_command, "cwd": "{task_dir}",
